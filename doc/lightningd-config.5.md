...
This address can be used in specific scenarios where a non-public address is preferred for incoming connections,
offering more control over the connection process.
...
This address is given to peers with whom a channel is already established, allowing for enhanced privacy.
This address is not used for initial connections but can be shared with trusted peers for future communications.

  An empty 'IPADDRESS' is a special value that clears any previously saved alternate announce addresses,
effectively resetting this setting. (Added in v24.08).
