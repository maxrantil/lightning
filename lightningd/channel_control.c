...
static void handle_our_alt_addr(struct lightningd *ld, struct channel *channel,
				const u8 *msg)
{
	u8 *our_alt_addr;

	if (!fromwire_channeld_our_alt_addr(tmpctx, msg, &our_alt_addr)) {
		channel_internal_error(channel,
				       "bad channeld_our_alt_addr %s",
				       tal_hex(channel, msg));
		return;
	}

	wallet_add_alt_addr(ld->wallet->db, &channel->peer->id,
			    (char *)our_alt_addr, true);
...
static unsigned channel_msg(struct subd *sd, const u8 *msg, const int *fds)
{
	...
	case WIRE_CHANNELD_OUR_ALT_ADDR:
		handle_our_alt_addr(sd->ld, sd->channel, msg);
		break;
