...
static void whitelist_peer_if_alt_addrs_exist(struct lightningd *ld,
					      struct peer *p)
{
...
void setup_peers(struct lightningd *ld)
{
	...
	for (p = peer_node_id_map_first(ld->peers, &it);
		...
		whitelist_peer_if_alt_addrs_exist(ld, p);
		setup_peer(p, delay > 0 ? delay : 0);
		delay++;
	}

	/* Send the whitelist to connectd if there are any */
	if (tal_count(ld->whitelisted_peers) > 0) {
